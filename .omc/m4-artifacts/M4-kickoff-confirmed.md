...
| APK size headroom for bootstrap zip (M3-S10) | **PASS** | Release APK 7.4MB (90.7% under 80MB gate). Combined budget: 7.4MB of 120MB = **73MB headroom** for bootstrap zip. Bootstrap zip target 30-50MB compressed (ralplan §6 M4 #1). Headroom is tight: 50MB zip + 7.4MB APK = 57.4MB, PASS but no margin for uncontrolled growth. |
...
| 4 | Upgrade path: app v1.0 → v1.1 with bootstrap-zip-content changes → installed packages migrate via reinstall manifest, no user data loss | `files/usr/` upgraded packages verified; user home dir (`$PREFIX/home`) intact after upgrade; no partial-state corruption | S05 (sub-test 4 corrupt usr/ recovery) | Covered by `test-bootstrap-install.sh` sub-test 3+4 |
...
1. **M4-S02** (fork + $PREFIX retargeting) — Bootstrap Build P0. Fork `termux/termux-packages` to `ImL1s/termux-packages` on branch `warp-mobile/main`; search-replace $PREFIX; produce `scripts/setup-warp-prefix.sh`; document conflict footprint. Owner: executor (opus).
   - 先決條件：macOS git + GitHub CLI available; no Docker required for M4-S02
   - 完成門檻：`ImL1s/termux-packages` fork exists on GitHub; `scripts/setup-warp-prefix.sh` runs on macOS; `grep -r 'com.termux' -- '*.sh' | wc -l` returns 0 in our fork
