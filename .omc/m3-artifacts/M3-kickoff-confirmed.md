...
**Invariant**: `warp_terminal → warpui` Cargo edge stays. `warp_terminal` crate itself is NOT modified by M3 (except DCS parser work in S05, which is additive). D2-lite has been superseded by Amendment 2 and is NOT revived.
...
**Honest dependency flag**: M3 is **more dependency-heavy on warp upstream** than M2 was. M2 added new files to warp-src; M3 must MODIFY existing `app/` + `warp_terminal` files. Cherry-pick velocity (Death-pit #2) is a genuine risk. If cherry-pick conflicts in `app/` consistently exceed budget, the upstream Warp HEAD must be pinned earlier rather than later.
