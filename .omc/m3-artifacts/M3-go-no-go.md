...
| M3-S06 | Bootstrap zsh_body.sh ship as APK asset + readable from PTY context | **PASS** | — | `42cba95` | 2 | `assets/warp/zsh_body.sh` 65K APK asset (`M3-S10-result.json:top_10_contributors[rank=4]`); atomic extraction + Gate 4b PTY-context exercise |
...
|---|---|---|---|
| 1 | PTY stream → cfg-gated `app::terminal::*` → correctly colored, line-wrapped `ls -la /system` (≥95% pixel similarity; ANSI 16-color dirs=blue + exec=green; line-wrap at col boundary) | S08 (primary) + S05 (SGR) + S04 (pipeline) | `.omc/m3-artifacts/M3-S08-result.json:gate.overall_pass=true`; `M3-S08-result.json:subtests.ls_real_pty.glyph_quads_observed=995`; `M3-S08-result.json:subtests.sgr_color_test.sgr_apply_count=4`; screenshot `.omc/m3-artifacts/M3-S08-ls-output.png` 186934 B. AC#5 toybox-color + AC#6 pixel-similarity deferred to M5 per prd.json M3-S08 amendments. |
